/***********************************************************************
 * โปรแกรมตัวอย่าง: ESP32 IoT with Blynk - DHT22, BH1750 & Relay Control
 * 
 * คำอธิบาย:
 * - อ่านค่าอุณหภูมิและความชื้นจาก DHT22 Temperature & Humidity Sensor
 * - อ่านค่าความเข้มแสงจาก BH1750 Light Intensity Sensor
 * - ควบคุม Relay 1 และ Relay 2 ผ่าน Blynk App
 * - แสดงผลบนจอ TM1638 Display Module (อุณหภูมิ-ความชื้น)
 * - แสดงผลใน Serial Monitor
 * - ส่งข้อมูลไปยัง Blynk Cloud Platform ผ่าน WiFi
 * 
 * Blynk Virtual Pins:
 *   V0 = อุณหภูมิจาก DHT22 (°C) - Output
 *   V1 = ความชื้นจาก DHT22 (%) - Output
 *   V2 = ความเข้มแสงจาก BH1750 (lux) - Output
 *   V3 = ควบคุม Relay 1 (0=OFF, 1=ON) - Input
 *   V4 = ควบคุม Relay 2 (0=OFF, 1=ON) - Input
 * 
 * การต่อฮาร์ดแวร์:
 * - ESP32 DevKit V1
 ...
 * 7. รับคำสั่งควบคุม Relay 1 & 2 จาก Blynk App (V3, V4)
 * 
 * การตั้งค่า Blynk App:
 * - V0: Gauge Widget (Temperature, 0-50°C)
 * - V1: Gauge Widget (Humidity, 0-100%)
 * - V2: Value Display (Light Intensity, 0-65535 lux)
 * - V3: Button Widget (Relay 1 Control, Switch Mode)
 * - V4: Button Widget (Relay 2 Control, Switch Mode)
 * 
 * หมายเหตุ:
 * - ต้องสร้าง Device Template ใน Blynk Console ก่อน
 * - ตั้งค่า Virtual Pins ใน Blynk App ตามที่ระบุข้างต้น
 * - Button Widget ตั้งเป็น Switch Mode (0/1)
 * 
...
// ===== ฟังก์ชัน BLYNK_WRITE สำหรับรับค่าจาก Blynk App =====
// ฟังก์ชันนี้จะถูกเรียกอัตโนมัติเมื่อมีการเปลี่ยนแปลงค่าใน Virtual Pin V3
BLYNK_WRITE(V3)
{
    // อ่านค่าจาก Virtual Pin V3 (0 = OFF, 1 = ON)
    int relay1State = param.asInt();
    
    // ควบคุม Relay 1 ตามค่าที่ได้รับ
    if (relay1State == 1) {
        // ค่าเป็น HIGH (1) = เปิด Relay 1
        board.relay1_On();
        Serial.println("[Blynk] Relay 1: ON");
    } else {
        // ค่าเป็น LOW (0) = ปิด Relay 1
        board.relay1_Off();
        Serial.println("[Blynk] Relay 1: OFF");
    }
}

// ฟังก์ชันนี้จะถูกเรียกอัตโนมัติเมื่อมีการเปลี่ยนแปลงค่าใน Virtual Pin V4
BLYNK_WRITE(V4)
{
    // อ่านค่าจาก Virtual Pin V4 (0 = OFF, 1 = ON)
    int relay2State = param.asInt();
    
    // ควบคุม Relay 2 ตามค่าที่ได้รับ
    if (relay2State == 1) {
        // ค่าเป็น HIGH (1) = เปิด Relay 2
        board.relay2_On();
        Serial.println("[Blynk] Relay 2: ON");
    } else {
        // ค่าเป็น LOW (0) = ปิด Relay 2
        board.relay2_Off();
        Serial.println("[Blynk] Relay 2: OFF");
    }
}
...
void setup()
{
    ...
    Serial.begin(115200);
    Serial.println("\n===== ESP32 IoT with Blynk - Sensor & Relay Control =====");

    // เริ่มต้น esp32_iot board (Buzzer, TM1638, Relay, ฯลฯ)
    board.begin();
    Serial.println("[OK] ESP32 IoT Board initialized");

    // ปิด Relay ทั้ง 2 ตัวตอนเริ่มต้น
    board.relay1_Off();
    board.relay2_Off();
    Serial.println("[OK] Relay 1 & 2 initialized (OFF)");
    ...
    if (Blynk.connected())
    {
        ...
        Serial.println("  V0 = Temperature (°C)");
        Serial.println("  V1 = Humidity (%)");
        Serial.println("  V2 = Light Intensity (lux)");
        Serial.println("  V3 = Relay 1 Control (0=OFF, 1=ON)");
        Serial.println("  V4 = Relay 2 Control (0=OFF, 1=ON)");
...
void loop()
{
    ...
    // ฟังก์ชันนี้จะจัดการการรับ-ส่งข้อมูลกับ Blynk Cloud
    Blynk.run();
    ...
    // หมายเหตุ: การควบคุม Relay จะทำงานอัตโนมัติผ่านฟังก์ชัน BLYNK_WRITE(V3) และ BLYNK_WRITE(V4)
    // เมื่อผู้ใช้กดปุ่มใน Blynk App (V3 หรือ V4)

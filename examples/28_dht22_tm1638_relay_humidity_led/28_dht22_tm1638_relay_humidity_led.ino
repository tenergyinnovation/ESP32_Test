/***********************************************************************
 * โปรแกรมตัวอย่าง: DHT22 + TM1638 + Relay Temperature Control with Humidity LED Indicator
 * 
 * รายละเอียด:
 ...
 * - แสดงระดับความชื้นผ่าน LED บน TM1638 (LED 1-7):
 *   * ความชื้น < 10%: เปิด LED 1 เพียงดวงเดียว
 *   * ความชื้น > 30%: เปิด LED 1-3
 *   * ความชื้น > 50%: เปิด LED 1-4
 *   * ความชื้น > 70%: เปิด LED 1-5
 *   * ความชื้น > 80%: เปิด LED 1-6
 *   * ความชื้น >= 90%: เปิด LED 1-7
 ...
 * - แสดงระดับความชื้นแบบ Bar Graph ด้วย LED
...
void setup()
{
  ...
  Serial.println(" with Humidity LED Indicator");
  Serial.println("=====================================");
  ...
  Serial.println("");
  Serial.println("การแสดงผล LED (ความชื้น):");
  Serial.println("- < 10%  : LED 1");
  Serial.println("- > 30%  : LED 1-3");
  Serial.println("- > 50%  : LED 1-4");
  Serial.println("- > 70%  : LED 1-5");
  Serial.println("- > 80%  : LED 1-6");
  Serial.println("- >= 90% : LED 1-7");
  Serial.println("=====================================");
  Serial.println("");
}
...
void loop()
{
  ...
  // แสดงสถานะความชื้นผ่าน LED บน TM1638 (LED 1-7)
  // ปิด LED ทั้งหมดก่อน (LED 1-8)
  for (int i = 1; i <= 8; i++) {
    board.tm1638_led(i, false);
  }
  
  // เปิด LED ตามระดับความชื้น
  if (humidity >= 90.0) {
    // ความชื้น >= 90%: เปิด LED 1-7
    for (int i = 1; i <= 7; i++) {
